    ...
    <div className="space-y-4 font-mono text-xs text-[#B984DF]">
      ...
      <NcursesFrame title="PHASE 02: TIME-SERIES MOMENTUM DELTAS & RANDOM FOREST" headerExtra="MODEL: [ TRAINED ]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
          ...
          <div className="border border-[#C095E4] px-3 py-1 bg-[#FFD1D4] text-[#B984DF] text-[11px]">
            ESTIMATORS: <span className="font-bold text-[#B984DF]">50</span> | MAX DEPTH: <span className="font-bold">10</span> | SEED: <span className="font-bold">42</span>
          </div>
        </div>
      </NcursesFrame>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <NcursesFrame title="SECTION 2.1: TIME-SERIES MOMENTUM DELTAS">
          <p className="text-[#C095E4] mb-3 text-[11px]">
            Calculated backward 10-tick shifts to measure acceleration of order book state.
          </p>
          <div className="space-y-2">
            <div className="border border-[#C095E4] p-2 bg-[#FFD1D4]">
              <div className="font-bold text-[#B984DF]">[ Micro_Price_Delta_10 ]</div>
              <code className="text-[#C095E4] text-[11px]">Micro_Price_Dollars - Micro_Price_Dollars.shift(10)</code>
            </div>
            <div className="border border-[#C095E4] p-2 bg-[#FFD1D4]">
              <div className="font-bold text-[#B984DF]">[ Deep_Imbalance_Delta_10 ]</div>
              <code className="text-[#C095E4] text-[11px]">Deep_Imbalance_Ratio - Deep_Imbalance_Ratio.shift(10)</code>
            </div>
          </div>
        </NcursesFrame>
        ...
        <NcursesFrame title="SECTION 2.2: STRICT CHRONOLOGICAL SPLIT">
          ...
          <div className="border border-[#C095E4] p-3 bg-[#FFD1D4] space-y-2">
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-[#B984DF]">TRAIN SET (MORNING 80%):</span>
              <span className="font-bold text-[#B984DF]">{datasetSummary?.phase2_train_rows || 7976} ROWS</span>
            </div>
            <div className="text-[#B984DF] font-bold tracking-tighter overflow-hidden whitespace-nowrap">
              {renderAsciiBar(0.8, 30)} 80%
            </div>
            <div className="flex justify-between items-center text-[11px] pt-1">
              <span className="text-[#C095E4]">TEST SET (AFTERNOON 20%):</span>
              <span className="font-bold text-[#B984DF]">{datasetSummary?.phase2_test_rows || 1994} ROWS</span>
            </div>
            <div className="text-[#C095E4] font-bold tracking-tighter overflow-hidden whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <NcursesFrame title="MODEL FEATURE IMPORTANCE HIERARCHY (SORTED DESCENDING)">
          <div className="space-y-3 font-mono text-xs">
            {features.map((item, idx) => (
              <div key={idx} className="border-b border-[#FFB7C5] pb-2">
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-[#B984DF]">#{idx + 1} {item.feature}</span>
                  <span className="font-bold text-[#B984DF]">{(item.importance * 100).toFixed(2)}%</span>
                </div>
                <div className="text-[#B984DF] tracking-wider">
                  {renderAsciiBar(item.importance, 28)}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 text-[11px] text-[#C095E4]">
        ...
        <NcursesFrame title="SCIKIT-LEARN CLASSIFICATION REPORT (AFTERNOON TEST SET)">
          <div className="border border-[#C095E4] p-3 bg-[#FFD1D4] text-[#B984DF] font-mono overflow-x-auto">
...
weighted avg       1.00      1.00      1.00      1994`}</pre>
          ...
          <div className="mt-3 border border-[#B984DF] p-2 text-[11px] text-[#FCEDF2] bg-[#B984DF] text-center font-bold uppercase">
